...
### 必需参数
- **源文件夹路径**：需要整理的文件所在目录
- **目标文件夹路径**：整理后存放文件的目录（默认为当前目录下的新文件夹）
...
- **重命名方式**：
  - 追加日期时间戳
    - **时间戳格式**：`YYYYMMDD_HHMMSS`（如：report_20240128_143022.txt）
    - 时间来源：使用文件修改时间或当前时间（询问用户选择）
  - 追加序号（如：photo_001.jpg, photo_002.jpg）
    - **序号规则**：询问用户是全局编号还是每个分类独立编号
      - 全局编号：所有文件从 001 开始递增
      - 分类独立编号：每个分类文件夹内从 001 开始
  - 追加日期时间戳 + 序号（如：report_20240128_143022_001.txt）
  - 保持原文件名
- **复制或移动**：
  - 复制（保留原文件）
  - 移动（删除原文件，默认复制）
...
请提供以下信息：
- 源文件夹路径：[从用户获取或使用当前目录]
- 目标文件夹名称：[从用户获取或默认为 "organized_files"]
- 分类方式：[从用户获取或默认为扩展名]
- 重命名方式：[从用户获取或默认为不重命名]
...
2. **目标目录已存在**：询问用户是否覆盖或使用新名称
3. **文件名冲突**：在重命名时检查目标文件是否存在，避免覆盖
...
**选项3：自动重命名**
```
目标文件 "images/photo.jpg" 已存在
自动重命名为 "images/photo_1.jpg"
```

**自动重命名规则：**
1. 尝试添加后缀 `_1`, `_2`, `_3`... 直到找到可用文件名
2. 示例：`photo.jpg` → `photo_1.jpg` → `photo_2.jpg` → ...
3. 如果原文件已有数字后缀，则递增该数字
...
执行：
1. 创建 images/, documents/, videos/, audio/, archives/, code/, others/ 文件夹
2. 将 .jpg/.png/.gif 复制到 images/
3. 将 .pdf/.docx/.txt 复制到 documents/
4. 将 .mp4/.avi 复制到 videos/
5. 将 .mp3/.wav 复制到 audio/
6. 将 .zip/.rar 复制到 archives/
7. 将 .py/.js/.html 复制到 code/
8. 其余文件复制到 others/
```
...
执行：
1. 检查文件修改时间
2. 创建嵌套目录：2023/12/, 2024/01/, 2024/02/...
3. 将文件复制到对应年/月文件夹：
   - report.pdf (修改于 2024-01-15) → 2024/01/
   - photo.jpg (修改于 2023-12-20) → 2023/12/
   - data.txt (修改于 2024-02-01) → 2024/02/
```
